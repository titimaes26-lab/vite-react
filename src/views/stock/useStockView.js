...
export function useStockView({ stock, setStock, cash, setCash, addTx, addToast, addDayStat, kitchen, supplierMode, setSupplierMode, pendingDeliveries, setPendingDeliveries, menu, restoLvN }) {
  ...
  const alertIds = new Set(
    menu.filter(d=>(d.unlockLevel??0)<=restoLvN).flatMap(d=>dec(d).map(i=>i.stockId))
  );
  const unlockedIds = new Set(
    menu.filter(d=>d.enabled!==false&&(d.unlockLevel??0)<=restoLvN).flatMap(d=>dec(d).map(i=>i.stockId))
  );
  const visibleStock = stock.filter(s=>unlockedIds.has(s.id));

  const [viewMode, setViewMode] = useState("cartes");
  const [collapsedCats, setCollapsedCats] = useState({});
  const [sortMode, setSortMode] = useState("urgence");

  const alerts = stock.filter(s=>alertIds.has(s.id)&&s.qty<=s.alert);
  ...
  const orderByForecast = () => {
    criticalIngredients.forEach(it=>{
      const qty = +(it.alert*6-it.qty-pendingQty(it.id)).toFixed(3);
      if(qty>0){
