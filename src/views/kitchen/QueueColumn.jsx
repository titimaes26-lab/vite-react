...
export const QueueColumn = memo(function QueueColumn({ queueByTable, queueLength, cookingLength, maxConcurrent, slotsLeft, chefOnShift, activeCommisCount, effectiveChefSpeed, upgDishCookTime, moveTicket, startDish, startAll }) {
  const { t: tl } = useLang();
        ...
        {queueGroups.map((tblQ,tIdx,arr)=>{
          ...
          return(
            <div key={tblQ.tableId||"nt"} style={{
              ...
              {tblQ.dishes.map((d,i)=>{
                const estSec = upgDishCookTime(d.prepTime||60,effectiveChefSpeed,activeCommisCount,d.cat||"");
