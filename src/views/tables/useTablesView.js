...
export function useTablesView({
  ...
  const checkout = (tid) => {
    ...
    setChallengeProgress&&setChallengeProgress(p=>({...p,served:(p.served||0)+1,revenue:p.revenue+total,tips:+((p.tips||0)+tip).toFixed(2),highRating:r>=4?(p.highRating||0)+1:(p.highRating||0),vip:t.group?.isVIP?Math.max(p.vip||0,1):p.vip||0}));
